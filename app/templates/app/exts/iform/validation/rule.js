...
define(
	...
	function (
		...
		// 6.依赖关系，第一个数值不大于第二个
		Rule.add("relation", "不符合依赖关系", function(value, text, first, second) {
			var firstNum = parseFloat($('#'+first).val());
			var secondNum = parseFloat($('#'+second).val());
		...
		// 7. 中文、英文、金额格式、电话（座机类）、url和email格式的校验
		_.each([
			["number", /^(-?\d+)(\.\d+)?$/, "只能输入数字"],
			["chinese", /^[\u0391-\uFFE5]+$/, "只能输入中文"],
